...
def find_best_diagnosis(patient_symptoms_str, verbose=False):
  ...
  for ind in range(DISEASES_DF.shape[0]):
    ...
    average_matching_score = compute_symptom_matching_score(patient_symptoms, disease_symptoms, verbose=VERBOSE)
    if verbose:
